...
motor_links: Motor =Motor(Port.A)
motor_mitte: Motor =Motor(Port.zeit_des_stopps)
motor_rechts: Motor =Motor(Port.C)
auto_sensor: TouchSensor =TouchSensor(Port.S1)
fortfahren: Touchsensor =TouchSensor(Port.S2)
...
def zeige_warte(texte: list):
    for text in texte:
        ...
        ev3.screen.draw_text(text=texte[0][0], x=60, y=110)
        halter()

def gebe_formatierten_text_aus(x: int, y: int, text: str, text_color: Color=Color.BLACK, background_color: Color=Color.WHITE, zeilenlaenge: int =21, zeilenhoehe: int =11): #ab 15 Zeilen grob ein Problem
    zaehler: int =0
    for i in text:
        spalte: int =(zaehler)%zeilenlaenge
        zeile: int =(zaehler-(zaehler % zeilenlaenge))/zeilenlaenge
...
def gebe_aus(text: str =" ", x: int =0, y: int =0, text_color: Color =Color.BLACK, background_color: Color =Color.WHITE, bubble_: bool =True, zeilenlaenge:int =25, zeilenhoehe:int =10):
    if bubble_==True:
...
def gebe_name():
    ...
    name: list =[" "]
    ausgewaehlt: str =""
    tastatur: list =["Q", "W", "E", "R", "T", "Z", "U", "I", "O",
              "P", "A", "S", "D", "F", "G", "H", "J", "K",
...
def bestenliste():
    with open("highscore.txt", "r") as file:
        sortierte_liste: list =[]
        bestenliste_eintraege: list =[]
        bestenliste_ganzer_inhalt: str =file.read()
        bestenliste_eintraege: list =bestenliste_ganzer_inhalt.split(";")
    ...
    ausgewaehlter_menuepunkt: int =0
    position: int =0
    while not Button.CENTER in ev3.buttons.pressed():
        ...
        for i in range(ausgewaehlter_menuepunkt*7, 7*(ausgewaehlter_menuepunkt+1)):
            try:
                rang: int =ausgewaehlter_menuepunkt+position+1
                line: str =str(rang)+". "+"".join(sortierte_liste[i])
...
# Menü (4)
while True:
    ausgewaehlter_menuepunkt: int =1
    while True:
        wait(200)
        ev3.screen.clear()
        gebe_aus(text=texte[1][0], x=30, y=5, text_color=Color.BLACK, background_color=Color.WHITE, bubble_=False)
        for i in range(1, len(texte[1])):
            if ausgewaehlter_menuepunkt==i:
                gebe_aus(text=texte[1][i], x=30, y=28+20*i, text_color=Color.WHITE, background_color=Color.BLACK, bubble_=False)
            else:
                gebe_aus(text=texte[1][i], x=30, y=28+20*i, text_color=Color.BLACK, background_color=Color.WHITE, bubble_=False)
        while not ((Button.RIGHT in ev3.buttons.pressed()) or (Button.LEFT in ev3.buttons.pressed()) or (Button.UP in ev3.buttons.pressed()) or (Button.DOWN in ev3.buttons.pressed()) or (Button.CENTER in ev3.buttons.pressed())):
            pass
        if (Button.DOWN in ev3.buttons.pressed()):
            ausgewaehlter_menuepunkt=(ausgewaehlter_menuepunkt+1)%(len(texte[1])-1)
            if ausgewaehlter_menupunkt=0:
                ausgewaehlter_menupunkt=1
            continue
    ...
    wait(500)
    ev3.speaker.beep()
    startgeschwindigkeit: int =-200
    durchschnittsgeschwindigkeit: int =200
    alle_geschwindigkeiten: list =[startgeschwindigkeit, startgeschwindigkeit, startgeschwindigkeit]
    zeit_fuer_neue_geschwindigkeit: int =randint(1, 5)
    nicht_verloren: bool =True
    startzeit: int =zeit_in_millisekunden()
    while nicht_verloren==True:
        ...
        punktzahl: int =int(zeit_in_millisekunden()-startzeit)
        ...
        while int(zeit_in_millisekunden()-startzeit)%zeit_fuer_neue_geschwindigkeit!=0:
            if (fortfahren.pressed()):
                zeit_des_stopps: int =zeit_in_millisekunden()
                motor_links.hold()
                motor_mitte.hold()
                motor_rechts.hold()
                ev3.screen.clear()
                ...
                wait(500)
                halter()
                zeit_des_weiterspielens: int =zeit_in_millisekunden()
